...
class RegisterDto {
    ...
    static _OPENAPI_METADATA_FACTORY() {
        return { email: { required: true, type: () => String, maxLength: 50, format: "email" }, firstName: { required: true, type: () => String, maxLength: 50 }, lastName: { required: true, type: () => String, maxLength: 50 }, password: { required: true, type: () => String, minLength: 8, maxLength: 50, pattern: "/^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[!@#$%^&*])[A-Za-z\\d!@#$%^&*]{8,}$/" }, role: { required: true, type: () => String, enum: ['DIRECTIVO', 'SENSOR', 'TECNICO', 'SERVICIOS_GENERALES'] } };
    }
}
...
__decorate([
    ...
    (0, swagger_1.ApiProperty)({ example: 'DIRECTIVO' }),
    __metadata("design:type", String)
